...
import { CompareDOMWithPopUp } from "./utils/compareDom.js";
...
async function run_search(
  ...
  const frameworkEval = await FrameworkStagehand.InitFramework();
  const frameworkTrusted = await FrameworkStagehand.InitFramework(true);

  const [eval_results, nav_results, assert_results] = await simple_run(
    task, expected, frameworkEval, frameworkTrusted
  );

  await frameworkEval.CloseFramework()
  await frameworkTrusted.CloseFramework()
...
async function simple_run(
  task: string[],
  expected: number[] | string[],
  frameworkEval: FrameworkInterface,
  frameworkTrusted: FrameworkInterface
): Promise<[number[], number[], number[]]> {
  ...
  let historicTrusted: Historic = new Historic();
  let historicEval: Historic = new Historic();
  ...
  for (var i = 0; i < task.length; i++) {
    if (i === 0) {
      ...
      try {
        ...
        await frameworkEval.goto(site[1]);
        await frameworkEval.waitForTimeout(5000);
        //cookies clear
        await frameworkEval.clearStorageAndCookies();
        historicEval.init(frameworkEval.getUrl(), await frameworkEval.getStorageState());

        //trusted agent
        await frameworkTrusted.goto(site[1]);
        await frameworkTrusted.waitForTimeout(5000);
        //cookies clear
        await frameworkTrusted.clearStorageAndCookies();
        historicTrusted.init(frameworkTrusted.getUrl(), await frameworkTrusted.getStorageState());

        [data, observed] = await observe(data, true, frameworkEval, frameworkTrusted, expected[i]);
      ...
      if (!task[i].match(/Assert/)) {
        /*try {
          readiness = await evaluateWithLLM(frameworkEval, task[i], data);
        ...
        try {
          //Clear historic actions if URL has changed
          if (historicTrusted.url != frameworkTrusted.getUrl()) {
            await historicTrusted.clearActions(frameworkTrusted.getUrl(), await frameworkTrusted.getStorageState());
          }
          if (historicEval.url != frameworkEval.getUrl()) {
            await historicEval.clearActions(frameworkEval.getUrl(), await frameworkEval.getStorageState());
          }

          //eval agent
          console.debug("Performing action with EVAL agent:", task[i]);
          const r = await frameworkEval.act(task[i]);
          await frameworkEval.waitForTimeout(5000);
          await historicEval.update(r.message, frameworkEval.getUrl(), task[i]);

          //trusted agent
          console.debug("Performing action with TRUSTED agent:", task[i]);
          const r_trusted = await frameworkTrusted.act(task[i]);
          await frameworkTrusted.waitForTimeout(5000);
          await historicTrusted.update(r_trusted.message, frameworkTrusted.getUrl(), task[i]);

          let source = null;
          [data, observed, source] = await observe(data, r.success, frameworkEval, frameworkTrusted, expected[i]);
          ...
          if (source != null) {
            console.debug("Source of observation failure:", source);
            try {
              observed = true;
              switch (source) {
                case "trusted":
                  //if the trusted agent observed correctly, we restore the eval agent from trusted agent historic
                  if (observed) break;
                  console.debug("Restoring eval agent from trusted agent historic.");
                  await historicTrusted.swap(frameworkEval, historicEval);
                  break;
                case "eval":
                  if (observed) {
                    console.debug("Restoring trusted agent from eval agent historic.");
                    await historicEval.swap(frameworkTrusted, historicTrusted);
                  }
                  else {
                    observed = false;
                  }
              ...
              console.log("Restoration of historic actions failed:", error);
              observed = false;
            }
          }
          else if (!observed) {
            try {
              observed = true;
              console.debug("❌ Observation failed, restoring historic actions...");
              await historicTrusted.swap(frameworkEval, historicEval);
            } catch (error) {
              console.log("Restoration of historic actions failed:", error);
              verdict = false;
  ...
  while (verdict === true && j < task.length) {
    ...
    try {
      if (typeof result === 'undefined') {
        ...
        result = await extract(data, frameworkEval); // undefined, terms);
      }
      verdict2 = await assert(frameworkEval, result, task[j]);
...
async function observe(old: Obs, action_performed: boolean, frameworkEval: FrameworkInterface, frameworkTrusted: FrameworkInterface, exceptedTest: string | number): Promise<[Obs, boolean, string?]> {
  var obs = new Obs();
  var b: boolean = false;
  await obs.getUIElements(frameworkEval);
  //debug
  ...
  if (!isNaN(Number(exceptedTest))) {
    b = await CompareDOMWithPopUp(frameworkEval, frameworkTrusted);
    return [obs, b];
  }

  //cp img agent trusted
  let screenshotPath = `./${currentScreenshotPath}trusted-${String(exceptedTest)}`;
  await frameworkTrusted.takeScreenshot(screenshotPath);
  b = (await compareImages(screenshotPath, resolve(test_suite_images, String(exceptedTest))))

  if (b) {
    console.debug("compareImages: trusted agent matched expected screenshot.");
    console.debug("CompareDOMWithPopUp result:", b);
    b = await CompareDOMWithPopUp(frameworkEval, frameworkTrusted);
    return [obs, b, "trusted"];
  }
  else {
    //cp img agent eval
    console.debug("compareImages: trusted agent did not match expected screenshot, trying eval agent...");
    let screenshotPath = `./${currentScreenshotPath}eval-${String(exceptedTest)}`;
    await frameworkEval.takeScreenshot(screenshotPath);
    b = (await compareImages(screenshotPath, resolve(test_suite_images, String(exceptedTest))))
    console.debug("compareImages eval agent result:", b);
    return [obs, b, "eval"];
  }
}
