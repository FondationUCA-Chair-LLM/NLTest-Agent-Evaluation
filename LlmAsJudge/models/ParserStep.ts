...
import { Tree } from "../utils/tokenizer.js";
...
export class ParserStep {
    ...
    //Parser assertion multiple ET/OU
    static async parseExpression(tokens: Tree,
        chunks: string[],
        llm: LLM,
        assert_all_chunks: (negative_assertion: boolean, chunks: string[], llm: LLM, assert_one_chunk: (chunk: string, chain: Runnable, inst?: string) => Promise<boolean>, inst?: string) => Promise<boolean>): Promise<boolean> {
        return await tokens.evaluate(chunks, llm, assert_all_chunks)
    }
}
